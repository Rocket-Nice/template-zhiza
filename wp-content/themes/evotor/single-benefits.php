...
if (empty($selectedPosts)) {
    $selectedPosts = [0]; // Если массив не заполнить значением, в подборку попадут первые 20 подборок, даже если они не выбраны.
}
...
<main class="page-content page-collection" data-post_id="<?= get_the_ID() ?>" data-post_type="<?= get_post_type() ?>">
    ...
    <?php if ( !empty($selectedPosts) && (count( $selectedPosts ) > $perPage) ) : ?>
        <section class="new-wrapper">
    ...
    <div class="new-wrapper content-padding">
        <section id="new_sub_main" class="new_sub" data-form="new_sub_form_main_page"
                 data-location="Форма подписки на странице коллекций"
        >
